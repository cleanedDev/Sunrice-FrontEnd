...
function CarrucelToursImg({images, open, setOpen}){
    return(
        <>
        <section className="w-full h-[90vh] fixed top-0 left-0 bg-gray-900 backdrop-blur-md bg-opacity-70 z-10 mt-[5rem] " aria-label="Tour image carousel">
                <button className="border fixed top-0 right-5 rounded-full z-20 p-2 ml-10 mt-10"  aria-label="Close carousel" onClick={()=> setOpen(!open)}>X</button>
                    <Swiper
                            ...
                            >
                              {images.map((image,index) => (
                                ...
                                <SwiperSlide key={index}  className="">
                                  <figure  className="lg:w-7/12 h-[90%] md:h-full mx-auto  md:w-11/12 flex items-center">
